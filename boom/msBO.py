    ...
    def query_candidate(self,
        ...
        for i in range(2):
            candidate, value = optimize_acqf( 
                ...
                bounds=botorch_bounds,   # (2, d_controls) only
                q=1,             # Number of candidates to generate
                num_restarts= 4,  # Number of restarts for optimization
                raw_samples = 128,   # Number of raw samples for initial candidate set
            )
            val_scalar = float(value.detach().cpu().item()) if torch.is_tensor(value) else float(value)
            if val_scalar > best_val:
                best_val = val_scalar
                best_candidate = candidate.detach().cpu().view(-1).numpy()
